...
urlpatterns = patterns('mongoadmin',
    ...
    url(r'^mongo/(?P<connection_name>[^/]+)/(?P<database_name>[^/]+)/(?P<collection_name>[^/]+)/(?P<pk>[a-zA-Z\d.%:\/\?\=\#\&\_]+)/$', views.UpdateDoc
    url(r'^mongo/(?P<connection_name>[^/]+)/(?P<database_name>[^/]+)/(?P<collection_name>[^/]+)/(?P<pk>[a-zA-Z\d.%:\/\?\=\#\&\_]+)/delete/$', views.De
)
